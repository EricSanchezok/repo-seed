...
Every decision — architectural or process — lives in one place: `docs/decisions/`. This is the unified decision log: architecture decisions and process decisions share the same format, the same lifecycle, and the same verifier. There is no second system.
