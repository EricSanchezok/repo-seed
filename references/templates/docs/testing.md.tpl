...
- **Unit** (`__TEST_COMMAND__`): fast tests that stay with the code area they exercise. Prefer edge cases, error paths, event ordering, and concurrency races.
...
- **End-to-end** (`__E2E_COMMAND__`): tests that boot the real entry point and verify the world — re-run the command or re-read the file externally, never the program's own claim.
