...
2. Write a decision record only when the change selects among meaningful alternatives; risk-boundary changes use an Approved spec in [__SPECS_PATH__/](__SPECS_PATH__/).
...
Durable architecture and process decisions use MADR in `__DECISIONS_PATH__/`. Follow the `repo-decisions` skill and [.repo-seed/update-strategy.md](.repo-seed/update-strategy.md) for the format. A superseded record is never rewritten; a new record supersedes it.
